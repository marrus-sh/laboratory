...
##  Object Initialization  ##
    ...
    current = Laboratory.Handlers.Account = {}
...
Upon completion, it should trigger an `Account.Received` event so that we can handle the data.
        ...
        Laboratory.Functions.requestFromAPI @meta.apiURL + "accounts/" + event.detail.id, @meta.accessToken, Laboratory.Events.Account.Received
...
We also send a request asking for the user's relationship to the account, so that we know whether we should display "Follow", "Unfollow", or "Block".
        ...
        Laboratory.Functions.requestFromAPI @meta.apiURL + "accounts/relationships?id=" + event.detail.id, @meta.accessToken, Laboratory.Events.Account.RelationshipsReceived

        return

    current.Requested.type = Laboratory.Events.Account.Requested.type
...
We use `Object.defineProperties` and `Object.seal` to keep our data safe.

        else
            Object.defineProperties @accounts, data.id,
                value: Object.seal do (data, stateData) ->
                    newData = {}
...
If our values changed, we should iterate over our components and feed them the new account data.
    ...
    current.Received.type = Laboratory.Events.Account.Received.type
...
We don't keep track of relationships in our store since it is rare we need to access more than one of them at a given time.
    ...
    current.RelationshipsReceived = (event) ->

        return unless event? and this? and event.type is current.RelationshipsReceived.type
        ...
        for relationship in event.detail.data
            ...
            for account in @interfaces.accounts[relationship.id]
                account.relationship = switch
                    when relationship.id is @meta.id then Laboratory.Symbols.Relationships.SELF
    ...
    current.RelationshipsReceived.type = Laboratory.Events.Account.RelationshipsReceived.type
    Object.freeze current.RelationshipsReceived
...
…And we're done!
    ...
    current.Removed.type = Laboratory.Events.Account.Removed.type
