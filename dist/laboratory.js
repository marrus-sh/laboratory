...
(function() {
  var Constructors, Enumerals, Events, Exposed, Handlers, Laboratory, generateEnumerals, handle, i, len, module, name, prop, ref, run, serverRequest, submodule,
  ...
  (function() {
    ...
    if ((code != null) && window.opener.Laboratory) {
      return window.opener.Laboratory.Authorization.Granted.dispatch({
        window: window,
        code: code
      });
    }
  ...
  Constructors.Post = function(data, accounts) {
    ...
    this.mediaAttachments = (function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        item = ref[i];
        results.push(new Constructors.MediaAttachment(item));
      }
      return results;
    })();
    this.mentions = (function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        item = ref[i];
        results.push(new Constructors.Mention(item));
      }
      return results;
    })();
    this.application = new Constructors.Application(data.application);
    return Object.freeze(this);
  };
  ...
  Constructors.Profile = function(data, origin, relationship) {
    ...
    if (relationship != null) {
      this.relationship = Enumerals.Relationship.fromValue(relationship) || this.relationship;
    }
    return Object.freeze(this);
  };
  ...
  generateEnumerals = function(data) {
    ...
    type = function(n) {
      return Constructors.Enumeral.call(this, n);
    };
  ...
  Handlers.Account = Object.freeze({
    RelationshipsRequested: handle(Events.Account.RelationshipsRequested, function(event) {
      ...
      serverRequest("GET", this.auth.api + "/accounts/relationships?id=" + id, null, this.auth.accessToken, Events.Account.RelationshipsReceived.dispatch);
    }),
    RelationshipsReceived: handle(Events.Account.RelationshipsReceived, function(event) {
      ...
      for (i = 0, len = data.length; i < len; i++) {
        ...
        relationship = Enumerals.Relationship.fromValue(Enumerals.Relationship.FOLLOWED_BY * relationships.followed_by + Enumerals.Relationship.FOLLOWING * relationships.following + Enumerals.Relationship.REQUESTED * relationships.requested + Enumerals.Relationship.BLOCKING * relationships.blocking + Enumerals.Relationship.SELF * (relationships.id === this.auth.me)) || Enumerals.Relationship.UNKNOWN;
    ...
    Requested: handle(Events.Account.Requested, function(event) {
      ...
      serverRequest("GET", this.auth.api + "/accounts/" + id, null, this.auth.accessToken, Events.Account.Received.dispatch);
      Events.Account.RelationshipsRequested.dispatch({
        id: id
      });
    }),
    Received: handle(Events.Account.Received, function(event) {
      ...
      profile = new Constructors.Profile(data, this.auth.origin);
    ...
    Follow: handle(Events.Account.Follow, function(event) {
      ...
      return serverRequest("POST", this.auth.api + "/accounts/" + id + (event.detail.value ? "/follow" : "/unfollow"), null, this.auth.accessToken, Events.Account.RelationshipsReceived.dispatch);
    }),
    Block: handle(Events.Account.Block, function(event) {
      ...
      return serverRequest("POST", this.auth.api + "/accounts/" + id + (event.detail.value ? "/block" : "/unblock"), null, this.auth.accessToken, Events.Account.RelationshipsReceived.dispatch);
    })
  });
  ...
  Handlers.Authorization = Object.freeze({
    ClientRequested: handle(Events.Authorization.ClientRequested, function(event) {
      ...
      authURL = a.href;
      serverRequest("POST", url + "/api/v1/apps", "client_name=" + encodeURIComponent(String(event.detail.name).replace(" ", "+")) + "&redirect_uris=" + encodeURIComponent(authURL) + "&scopes=read+write+follow", null, Events.Authorization.ClientReceived.dispatch, {
        url: url,
        redirect: authURL
      });
    }),
    ...
    Requested: handle(Events.Authorization.Requested, function(event) {
      var a, accessToken, authURL, clientID, clientSecret, redirect, ref, url;
      a = document.createElement("a");
      a.href = event.detail.url;
      url = a.origin;
      a.href = event.detail.redirect || "";
      ...
      if (localStorage.getItem("Laboratory | " + url)) {
        ref = localStorage.getItem("Laboratory | " + url).split(" ", 4), redirect = ref[0], clientID = ref[1], clientSecret = ref[2], accessToken = ref[3];
      }
      if (!((redirect && (event.detail.redirect == null) || redirect === authURL) && (clientID != null) && (clientSecret != null))) {
        Events.Authorization.ClientRequested.dispatch({
          url: url,
          redirect: authURL,
      ...
      if (accessToken) {
        ...
        serverRequest("GET", this.auth.api + "/accounts/verify_credentials", null, this.auth.accessToken, Events.Authorization.Verified.dispatch);
        return;
      }
    ...
    Granted: handle(Events.Authorization.Granted, function(event) {
      ...
      serverRequest("POST", this.auth.origin + "/oauth/token", "client_id=" + this.auth.clientID + "&client_secret=" + this.auth.clientSecret + "&redirect_uri=" + encodeURIComponent(this.auth.redirect) + "&grant_type=authorization_code&code=" + event.detail.code, null, Events.Authorization.Received.dispatch);
    }),
    Received: handle(Events.Authorization.Received, function(event) {
      ...
      localStorage.setItem("Laboratory | " + this.auth.origin, this.auth.redirect + " " + this.auth.clientID + " " + this.auth.clientSecret + " " + this.auth.accessToken);
      serverRequest("GET", this.auth.api + "/accounts/verify_credentials", null, this.auth.accessToken, Events.Authorization.Verified.dispatch);
    }),
    Verified: handle(Events.Authorization.Verified, function(event) {
      var ref;
      if (!isFinite((ref = event.detail.data) != null ? ref.id : void 0)) {
        localStorage.setItem("Laboratory | " + this.auth.origin, "");
        Events.Authorization.Requested({
          url: this.auth.origin,
          clientID: this.auth.clientID,
      ...
      Events.Account.Received.dispatch({
  ...
  Handlers.Composer = Object.freeze({
    UploadRequested: handle(Events.Composer.UploadRequested, function(event) {
      ...
      serverRequest("POST", this.auth.api + "/media", form, this.auth.accessToken, Events.Composer.UploadReceived.dispatch);
    }),
    ...
    Requested: handle(Events.Composer.Requested, function(event) {
      ...
      serverRequest("POST", this.auth.api + "/statuses", form, this.auth.accessToken, Events.Status.Received.dispatch);
    }),
    Requested: handle(Events.Composer.Requested, function(event) {
      var index;
      index = 0;
  ...
  Handlers.Status = Object.freeze({
    ...
    Received: handle(Events.Status.Received, function(event) {
      ...
      for (i = 0, len = timelinesToUpdate.length; i < len; i++) {
        ...
        Events.Timeline.Received.dispatch({
          data: [event.detail.data],
          params: {
            name: name
          }
    ...
    Follow: handle(Events.Status.SetReblog, function(event) {
      ...
      return serverRequest("POST", this.auth.api + "/statuses/" + id + (event.detail.value ? "/reblog" : "/unreblog"), null, this.auth.accessToken, Events.Status.Received.dispatch);
    }),
    SetFavourite: handle(Events.Status.SetFavourite, function(event) {
      ...
      return serverRequest("POST", this.auth.api + "/statuses/" + id + (event.detail.value ? "/favourite" : "/unfavourite"), null, this.auth.accessToken, Events.Status.Received.dispatch);
    }),
    Deletion: handle(Events.Status.Deletion, function(event) {
      ...
      for (i = 0, len = timelinesToUpdate.length; i < len; i++) {
        name = timelinesToUpdate[i];
        results.push(Events.Timeline.Received.dispatch({
          data: [
            {
              id: id
            }
  ...
  Handlers.Timeline = Object.freeze({
    Requested: handle(Events.Timeline.Requested, function(event) {
      ...
      serverRequest("GET", url, null, this.auth.accessToken, Events.Timeline.Received.dispatch, {
        name: name
      });
    }),
    Received: handle(Events.Timeline.Received, function(event) {
      ...
      for (i = 0, len = data.length; i < len; i++) {
        ...
        if (ref1 = item.id, indexOf.call(postOrder, ref1) < 0) {
          post = item.type === "follow" ? new Constructors.Follow(item, this.accounts) : new Constructors.Post(item, this.accounts);
  ...
  for (prop in Exposed) {
    Object.defineProperty(Laboratory, prop, {
      get: (function() {
        return Exposed[prop];
      }),
      enumerable: true
